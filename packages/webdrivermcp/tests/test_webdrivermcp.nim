...
suite "webdrivermcp scenarios":
  ...
  test "Take a base64 screenshot of an element":
    let srv = startServer()
    defer: srv.close()
    let url = "http://127.0.0.1:" & $gMockPort
    discard srv.mcpCall(1, "wd_new_web_driver", %*{"url": url})
    let sid = getText(srv.mcpCall(2, "wd_create_session", %*{}), "text")
    let r = srv.mcpCall(3, "wd_take_screen_shot_base64", %*{"session_id": sid, "css_selector": "button"})
    check "iVBORw0KGgo" in getText(r, "text")

  test "Get the visible text of an element":
    let srv = startServer()
    defer: srv.close()
    let url = "http://127.0.0.1:" & $gMockPort
    discard srv.mcpCall(1, "wd_new_web_driver", %*{"url": url})
  ...
  test "Get the value of an element":
    let srv = startServer()
    defer: srv.close()
    let url = "http://127.0.0.1:" & $gMockPort
    discard srv.mcpCall(1, "wd_new_web_driver", %*{"url": url})
    let sid = getText(srv.mcpCall(2, "wd_create_session", %*{}), "text")
    let r = srv.mcpCall(3, "wd_value", %*{"session_id": sid, "css_selector": "input"})
    check "mock-value" in getText(r, "text")
