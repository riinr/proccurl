...
proc work(idx: int; res: ptr UncheckedArray[int64]): void {.gcsafe.} =
  res[idx] = getMonoTime().ticks
  ...
  proc main(): void =
    ...
    m.awaitAll:
      for i in 0..<MAX_ITEMS:
        ...
        m.spawn work(i, res_all)
        sent_all[i] = getMonoTime().ticks
      tasksSent = getMonoTime().ticks
    ...
    echo "Total sending:\t", (tasksSent - send_all[0]).ns,   ((tasksSent - send_all[0]) div MAX_ITEMS).ns, "/task\t", "To schedule tasks"

    range = 1000
    let (st11, nd21, rd31, th41, th51) = top_items(send_all, res_all, range, MAX_ITEMS)
    ...
    echo "Snd+Join: \t", (ta - send_all[0]).ns, ((ta - send_all[0]) div MAX_ITEMS).ns, "/task\t", "Send + Join"
